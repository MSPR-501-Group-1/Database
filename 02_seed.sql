...
SELECT
  ...
  (date_trunc('day', gs.day_ref) + interval '04:05:00')::timestamp AS started_at,
  (date_trunc('day', gs.day_ref) + interval '04:18:00')::timestamp AS ended_at,
  CASE
    WHEN EXTRACT(DOW FROM gs.day_ref) IN (0, 6) THEN 'REJECTED'
    WHEN EXTRACT(DOW FROM gs.day_ref) = 3 THEN 'TRANSFORMED'
    ELSE 'LOADED'
  END::pipeline_status_enum AS status,
  (18000 + ((EXTRACT(DOY FROM gs.day_ref)::int % 6) * 450))::int AS records_extracted,
  (17000 + ((EXTRACT(DOY FROM gs.day_ref)::int % 6) * 390))::int AS records_loaded,
  (350 + ((EXTRACT(DOY FROM gs.day_ref)::int % 5) * 45))::int AS records_rejected,
  CASE
  ...
  GREATEST(
    ...
    ROUND(
      ...
      * (
        tpl.base_fail_ratio
        + CASE
            WHEN EXTRACT(DOW FROM gs.day_ref) IN (1, 2) THEN 0.015
            ELSE 0.006
          END
      )
    )::int
...
CROSS JOIN (
  VALUES
    ...
    ('B', 'user_metrics', 'RANGE_CHECK',       'heart_rate_avg BETWEEN 30 AND 220',   9800,  0.013, time '04:24:00'),
...
SELECT
  ...
  tpl.anomaly_table,
  tpl.field_name,
  tpl.record_identifier_prefix || '_' || to_char(gs.day_ref, 'YYYYMMDD') AS record_identifier,
  tpl.original_value,
  (date_trunc('day', gs.day_ref) + tpl.detect_time)::timestamp AS detected_at,
  tpl.severity,
  CASE
    WHEN tpl.suffix = 'D' AND EXTRACT(DOW FROM gs.day_ref) IN (2, 4) THEN TRUE
    ELSE FALSE
  END AS is_resolved,
  CASE
    WHEN tpl.suffix = 'D' AND EXTRACT(DOW FROM gs.day_ref) IN (2, 4) THEN 'Resolved by data steward during seed replay'
    ELSE NULL
  END AS resolution_action,
  'CHK_FIX_' || to_char(gs.day_ref, 'YYYYMMDD') || '_' || tpl.suffix AS check_id,
  'ETL_FIX_' || to_char(gs.day_ref, 'YYYYMMDD') AS execution_id
...
CROSS JOIN (
  VALUES
    ...
    ('B', 'user_metrics', 'user_metrics', 'heart_rate_avg', 'MET_FIX_HIGH',  '238',        'HIGH',   time '04:32:00'),
