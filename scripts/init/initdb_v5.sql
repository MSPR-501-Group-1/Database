...
CREATE TABLE health_goal(
   goal_id VARCHAR(50) DEFAULT gen_random_uuid(),
   label VARCHAR(50),
...
CREATE TABLE ingredients(
   ingredients_id VARCHAR(50) DEFAULT gen_random_uuid(),
   name VARCHAR(50),
...
CREATE TABLE recipe(
   recipe_id VARCHAR(50) DEFAULT gen_random_uuid(),
   title VARCHAR(50),
...
CREATE TABLE exercise(
   exercise_id VARCHAR(50) DEFAULT gen_random_uuid(),
   name VARCHAR(50),
...
CREATE TABLE connected_device(
   device_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE data_source(
   source_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE etl_execution(
   execution_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE data_quality_check_(
   check_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE data_anomaly(
   anomaly_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE feature(
   feature_id VARCHAR(50) DEFAULT gen_random_uuid(),
   name VARCHAR(50),
   description VARCHAR(200),
   category VARCHAR(50),
...
CREATE TABLE role(
   role_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE organization(
   organization_id VARCHAR(50) DEFAULT gen_random_uuid(),
   name VARCHAR(50),
...
CREATE TABLE subscription_plan(
   plan_id VARCHAR(50) DEFAULT gen_random_uuid(),
   name VARCHAR(50),
   description VARCHAR(50),
...
CREATE TABLE user_(
   user_id VARCHAR(50) DEFAULT gen_random_uuid(),
   email VARCHAR(50),
...
CREATE TABLE subscription(
   subscription_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE payment_transaction(
   transaction_id VARCHAR(50)DEFAULT gen_random_uuid(),
...
CREATE TABLE meal(
   meal_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE workout_session(
   session_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE exercice_details(
   exercice_details_id VARCHAR(50) DEFAULT gen_random_uuid(),
   sets INT,
   reps INT,
...
CREATE TABLE biometric_measure(
   measure_id VARCHAR(50) DEFAULT gen_random_uuid(),
   type VARCHAR(50),
   value_ INT,
...
CREATE TABLE ai_recommendation(
   recommendation_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE subscription_history(
   history_id VARCHAR(50) DEFAULT gen_random_uuid(),
...
CREATE TABLE pairs(
   ...
   FOREIGN KEY(user_id) REFERENCES user_(user_id) ON DELETE CASCADE,
   FOREIGN KEY(device_id) REFERENCES connected_device(device_id) ON DELETE CASCADE
);
...
CREATE TABLE records(
   ...
   FOREIGN KEY(device_id) REFERENCES connected_device(device_id) ON DELETE CASCADE,
   FOREIGN KEY(measure_id) REFERENCES biometric_measure(measure_id) ON DELETE CASCADE
);
...
CREATE TABLE gets(
   ...
   FOREIGN KEY(goal_id) REFERENCES health_goal(goal_id) ON DELETE CASCADE,
   FOREIGN KEY(metric_id) REFERENCES user_metrics(metric_id) ON DELETE CASCADE
);
